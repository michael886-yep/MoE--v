...
data = pd.read_csv("D://2//dongmoe2//12//canchafenlei//12dong2can.csv")  # 替换为您的CSV文件路径
...
# 找到每个样本预测的类别
val_predictions = pd.DataFrame(valPredict, columns=["class"]) #val_Predicted_Class
val_predictions.index = range(len(val_predictions))  # 设置索引为顺序索引

# 保存验证集预测结果
val_predictions.to_excel("D://2//dongmoe2//12//canchafenlei//12canval.xls", index=False)
