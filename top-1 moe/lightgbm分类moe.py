...
# 创建LightGBM模型
model = LGBMClassifier(n_estimators=50, random_state=seed, learning_rate=0.1)  # 替换为LightGBM
...
# 找到每个样本预测的类别
val_predictions = pd.DataFrame(valPredict, columns=["val_Predicted_Class"])
val_predictions.index = range(len(val_predictions))  # 设置索引为顺序索引

# 保存验证集预测结果
val_predictions.to_excel("D://2//jingmoe//3//fen//3jingval.xls", index=False)

print("验证集预测结果已保存为 '6jingval.xls'")